package com.grobster.game {
	...
	object NumberMatch {
		def listMatch[A](tries: List[A], master: List[A]): Int = {
			@scala.annotation.tailrec
			def _count(tries: List[A], master: List[A], count: Int): Int = tries match {
				case Nil => count
...
import com.grobster.util._
	object Tester {
		def main(args: Array[String]): Unit = {
			...
			val winnings = lotteryNumbers.map(x => NumberMatch.listMatch(x, lottery.getWinningNumbers))
